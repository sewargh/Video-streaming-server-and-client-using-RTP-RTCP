...
int main(int argc, char const *argv[])
{
    ...
    int opt = 1;
    // use sockaddr_in ?
    struct sockaddr_in ServerAddr,SenderAdd; //socket file descriptor for the server to connect to.
    ...
    while(1)
        {
            ...
            AddSenderLen = sizeof (SenderAdd);
            int BytesRead;
            cout<<"Waiting on port " << PortNum << endl;
            BytesRead = recvfrom(Socket_Server,Buffer,10000,0,(struct sockaddr*)&SenderAdd,(socklen_t*)&AddSenderLen);
            send(Socket_Server, ACKrtp.data, sizeof(ACKrtp.data), 0);
